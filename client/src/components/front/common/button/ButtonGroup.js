...
let buttonId = 0;
export default function ButtonGroup({classes, name, items = [], toggleElementIndex = 0, selected = '', onclickButton}) {
	...
	return (
		<>
			<span className={classNames("ctx-isolate ctx-inline-flex ctx-rounded-md ctx-shadow-sm", classes)}>
				...
				{items.length && items.map((item, index) => {
					if (index === 0) {
						return (
							<button
								id={buttonId++}
								onClick={(e) => onclickButton(e, toggleElementIndex)}
								type="button"
								name={name}
								value={item.type}
								className={getClassNames(buttonCSS[0], item)}
							>
								{item.text}
							</button>
						)
						...
						return (
							<button
								...
								value={item.type}
								name={name}
								className={getClassNames(buttonCSS['end'], item)}
							>
								{item.text}
							</button>
						)
						...
						return (
							<button
								...
								value={item.type}
								name={name}
								className={getClassNames(buttonCSS['normal'], item)}
							>
								{item.text}
							</button>
						)
