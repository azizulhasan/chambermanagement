...
export default function Notify() {
	...
	const {showNotice, noticeMessage, noticeDelay} = useSelector(state => state.modal)
	useEffect(() => {
							...
							<div className="ctx-p-4">
								<div className="ctx-flex ctx-items-start">
									<div className="ctx-flex-shrink-0">
										<CheckCircleIcon className="ctx-h-6 ctx-w-6 ctx-text-green-400" aria-hidden="true" />
									</div>
									<div className="ctx-ml-3 ctx-w-0 ctx-flex-1 ctx-pt-0.ctx-5">
										<p className="ctx-mt-1 ctx-text-sm ctx-text-gray-500">{noticeMessage}</p>
									</div>
									<div className="ctx-ml-4 ctx-flex ctx-flex-shrink-0">
										...
										>
											...
											<XMarkIcon className="ctx-h-5 ctx-w-5" aria-hidden="true" />
										</button>
									</div>
								</div>
							</div>
