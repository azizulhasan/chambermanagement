...
export default function Modal({children, showModal, closeModal, modalSize = 'sm:w-3/12', classes = ''}) {
	...
	return (
		<Transition.Root show={showModal} as={Fragment}>
			<Dialog as="div"  className="ctx-relative ctx-z-10" onClose={closeModal}>
				<Transition.Child
					as={Fragment}
				...
				<div className="ctx-fixed ctx-inset-0 ctx-z-10 ctx-overflow-y-auto">
					<div
						className="ctx-flex ctx-min-h-full ctx-items-end ctx-justify-center ctx-p-4 ctx-text-center sm:ctx-items-center sm:ctx-p-0">
						<Transition.Child
							as={Fragment}
							enter="ctx-ease-out ctx-duration-300"
						...
						>
							<Dialog.Panel className={classNames(modalClasses, classes )}>
								<div
									className="ctx-absolute ctx-top-0 ctx-right-0 ctx-hidden ctx-pt-4 ctx-pr-4 sm:ctx-block">
									<button
										type="button"
									...
									>
										<span className="ctx-sr-only">Close</span>
										<XMarkIcon className="ctx-h-6 ctx-w-6" aria-hidden="true"/>
									</button>
								</div>
								{children}
							</Dialog.Panel>
